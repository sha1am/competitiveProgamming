...
const int N=0;
 ...
 void printArrItem(struct item arr[],int n){
 	for( int i=0;i<n;i++){
 ...
 double fractionalKnapsack(double w, item arr[],int n){
 	...
 	for( int i=0;i<n;i++){
 		...
 		if(remWeight-arr[i].weight>=0){
 			...
 			totProfit+= (remWeight/arr[i].weight)*arr[i].value;
 			break;
 		}
 	}
 	return totProfit;
...
void solve(){
	...
	double w=50;
	cout<<" the largest profit that can be earned is "<<fractionalKnapsack(w,arr,n);
}
signed main(){
