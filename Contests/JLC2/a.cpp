...
void solve(){
	...
	cin>>d.dsa>>d.toc>>d.dm;
	cin>>s.dsa>>s.toc>>s.dm;

	d.totMarks=d.dsa+d.toc+d.dm;
	s.totMarks=s.dsa+s.toc+s.dm;

	debug(d.totMarks);
	debug(s.totMarks);

	int flag=-1; //0-d,1-s,2-tie

	if(d.totMarks>s.totMarks){
		flag=0;
	}
	else if(s.totMarks>d.totMarks){
		flag=1;
	}
	else{
		//tie condition
