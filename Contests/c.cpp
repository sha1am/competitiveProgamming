...
int totalSum = 0;
// returns the sum of all extremas
void  sumExtrema(vector<int> arr, int arrSize) {
	int sum = 0;
	rep(i, 1, arr.size() - 1) {
...
void recursivePermutationFunction(vector<int > v1, vector<int> tempVec , int frameSize, int pos) {

	//base condition
	if (pos == frameSize) {
		//print the arrray
		debug(tempVec);
		sumExtrema(tempVec,frameSize);
...
void printAllPermutations(vector<int> v1, int frameSize) {
...
void solve()
{
	//refresh totalSum at each iteration
	totalSum=0;
